...
class GestorAlianzas {
    ...
    responderPropuesta(propuestaId, empresaRespondiente, respuesta, detallesRespuesta) {
        ...
        switch (respuesta) {
            ...
            case 'rechazar':
                ...
                propuesta.fechaRespuesta = fechaActual;
                propuesta.motivoRechazo = (detallesRespuesta === null || detallesRespuesta === void 0 ? void 0 : detallesRespuesta.motivoRechazo) || 'No se especificó motivo';
                // Guardar la propuesta actualizada
                this.propuestas.set(propuestaId, propuesta);
                return propuesta;
            case 'contrapropuesta':
                if (!(detallesRespuesta === null || detallesRespuesta === void 0 ? void 0 : detallesRespuesta.contrapropuesta)) {
                ...
                const contrapropuesta = this.crearContrapropuesta(propuesta, empresaRespondiente, detallesRespuesta.contrapropuesta);
     ...
     */
    crearAlianzaDesdeAceptacion(propuesta, empresaAceptante) {
        ...
        if (propuesta.empresasObjetivo.length > 1) {
            ...
            const todasAceptaron = propuesta.empresasObjetivo.every(empresaId => {
                ...
                return Array.from(this.propuestas.values()).some(p => p.id !== propuesta.id && // No es la misma propuesta
        ...
        const alianza = {
            ...
            estado: competitionTypes_1.EstadoAlianza.ACTIVA,
            condiciones,
            beneficios,
     ...
     */
    crearContrapropuesta(propuestaOriginal, empresaContraproponente, detallesContrapropuesta) {
    ...
    actualizarAlianza(alianzaId, diasTranscurridos = 1) {
        ...
        if (alianza.fechaInicio + (alianza.duracion * 24 * 60 * 60 * 1000) <= fechaActual) {
            ...
            if (alianza.renovacionAutomatica) {
                ...
                // Terminar la alianza
                alianza.estado = competitionTypes_1.EstadoAlianza.TERMINADA;
        ...
        if (alianza.nivelConfianza < 30 && alianza.estado === competitionTypes_1.EstadoAlianza.ACTIVA) {
            alianza.estado = competitionTypes_1.EstadoAlianza.DETERIORADA;
            // Registrar evento
            alianza.historialEventos.push({
                fecha: fechaActual,
        ...
        if (alianza.nivelConfianza > 50 && alianza.estado === competitionTypes_1.EstadoAlianza.DETERIORADA) {
            alianza.estado = competitionTypes_1.EstadoAlianza.ACTIVA;
            // Registrar evento
            alianza.historialEventos.push({
                fecha: fechaActual,
     ...
     */
    actualizarCumplimientoCondiciones(alianza, diasTranscurridos) {
     ...
     * @param diasTranscurridos Días transcurridos
     */
    actualizarBeneficiosRealizados(alianza, diasTranscurridos) {
        ...
        const cumplimientoPromedio = alianza.condiciones.reduce((sum, condicion) => sum + condicion.cumplimiento, 0) / Math.max(1, alianza.condiciones.length);
        ...
        const beneficioTotal = alianza.beneficios.reduce((sum, beneficio) => sum + beneficio.valorRealizado, 0);
        // Registrar evento cada 25% de beneficio total realizado
        const beneficioEstimadoTotal = alianza.beneficios.reduce((sum, beneficio) => sum + beneficio.valorEstimado, 0);
        const porc;
        (Content);
        truncated;
        due;
...
exports.GestorAlianzas = GestorAlianzas;
