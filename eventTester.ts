...
import { ValidadorSistemaEventos } from './eventValidator';
...
const configuracionTest: ConfiguracionEventos = {
...
const estadoJuegoTest: EstadoJuego = {
  tiempoJuego: 0, // Día 0
...
export const ejecutarPruebasEventos = () => {
  console.log("=== INICIANDO PRUEBAS DEL SISTEMA DE EVENTOS DINÁMICOS ===");
  
  // Crear validador
  const validador = new ValidadorSistemaEventos();
  ...
  const sistemaEventos = new SistemaEventosDinamicos(
    mockSistemaEconomico,
    mockSistemaProduccion,
    mockSistemaLogistica,
    mockSistemaNotificaciones,
    configuracionTest
  );
  ...
  const resultadoCompleto = validador.validacionCompleta(
    eventosTest,
    configuracionTest,
    sistemaEventos,
    estadoJuegoTest
