...
exports.GestorNegociaciones = void 0;
const fusionesTypes_1 = require("./fusionesTypes");
/**
 * Clase que gestiona las negociaciones de fusiones y adquisiciones
 */
class GestorNegociaciones {
    constructor(gestorValoracion) {
        this.negociaciones = new Map();
        this.ofertas = new Map();
        this.acuerdos = new Map();
    ...
    iniciarNegociacion(empresaIniciadoraId, empresaObjetivoId, tipoOperacion, esHostil = false) {
        ...
        const negociacion = {
            ...
            empresaIniciadoraId,
            empresaObjetivoId,
            estado: fusionesTypes_1.EstadoNegociacion.IDENTIFICACION,
            ofertas: [],
     ...
     */
    realizarValoracion(negociacionId, datosAdicionales) {
        ...
        if (negociacion.estado !== fusionesTypes_1.EstadoNegociacion.IDENTIFICACION) {
            throw new Error(`Estado incorrecto para valoración: ${negociacion.estado}`);
        }
        // Realizar la valoración
        const valoracion = this.gestorValoracion.valorarEmpresa(negociacion.empresaObjetivoId, undefined, datosAdicionales);
        // Actualizar la negociación
        negociacion.valoracion = valoracion;
        negociacion.estado = fusionesTypes_1.EstadoNegociacion.VALORACION;
        negociacion.fechaUltimaActividad = Date.now();
        // Añadir nota
        this.agregarNotaNegociacion(negociacionId, `Valoración realizada: ${valoracion.valorEstimadoMin.toLocaleString()} - ${valoracion.valorEstimadoMax.toLocaleString()} créditos (${valoracion.metodoPrincipal})`);
    ...
    presentarOfertaInicial(negociacionId, oferta) {
        ...
        // Verificar que está en estado adecuado
        if (negociacion.estado !== fusionesTypes_1.EstadoNegociacion.VALORACION) {
        ...
        negociacion.estado = fusionesTypes_1.EstadoNegociacion.OFERTA_INICIAL;
        negociacion.fechaUltimaActividad = fechaActual;
        // Añadir nota
        this.agregarNotaNegociacion(negociacionId, `Oferta inicial presentada: ${nuevaOferta.precioOfertado.toLocaleString()} créditos (${nuevaOferta.terminosFinanciacion})`);
        // Guardar la negociación actualizada
        this.negociaciones.set(negociacionId, negociacion);
        return negociacion;
    }
    ...
    evaluarOferta(ofertaId) {
        ...
        switch (oferta.terminosFinanciacion) {
            ...
            case fusionesTypes_1.TerminoFinanciacion.DEUDA:
                ...
                break;
            case fusionesTypes_1.TerminoFinanciacion.MIXTO:
        ...
        if (precioAdecuado) {
            ...
            if (oferta.terminosFinanciacion === fusionesTypes_1.TerminoFinanciacion.EFECTIVO) {
                probabilidadAceptacion += 15;
            }
            else if (oferta.terminosFinanciacion === fusionesTypes_1.TerminoFinanciacion.ACCIONES) {
                probabilidadAceptacion -= 5;
            }
            else if (oferta.terminosFinanciacion === fusionesTypes_1.TerminoFinanciacion.DEUDA) {
                probabilidadAceptacion -= 10;
            }
    ...
    responderOferta(ofertaId, respuesta, contraoferta) {
        ...
        switch (respuesta) {
            case 'aceptar':
                ...
                negociacion.estado = fusionesTypes_1.EstadoNegociacion.ACUERDO_PRELIMINAR;
                negociacion.fechaUltimaActividad = fechaActual;
                // Añadir nota
                this.agregarNotaNegociacion(negociacionId, `Oferta aceptada: ${oferta.precioOfertado.toLocaleString()} créditos`);
                break;
            case 'rechazar':
                // Actualizar estado de la oferta
                ...
                if (!hayOfertasPendientes) {
                    negociacion.estado = fusionesTypes_1.EstadoNegociacion.RECHAZADA;
                }
                negociacion.fechaUltimaActividad = fechaActual;
                // Añadir nota
                this.agregarNotaNegociacion(negociacionId, `Oferta rechazada: ${oferta.precioOfertado.toLocaleString()} créditos`);
                break;
            case 'contraoferta':
                if (!contraoferta) {
...
exports.GestorNegociaciones = GestorNegociaciones;
