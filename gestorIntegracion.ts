 ...
 */
export class GestorIntegracion {
  private procesos: Map<string, ProcesoIntegracion> = new Map();
   ...
   */
  public iniciarProcesoIntegracion(
    negociacion: NegociacionMA,
    acuerdo: AcuerdoMA
  ): ProcesoIntegracion {
    ...
    const proceso: ProcesoIntegracion = {
      id,
      ...
      costoAcumulado: 0,
      estado: EstadoIntegracion.EN_PROGRESO,
      etapasIntegracion,
   ...
   */
  private calcularDuracionEstimada(acuerdo: AcuerdoMA): number {
    // Base: 30 días
    let duracion = 30;
    
    // Ajustar según desafíos de integración
    const desafiosTotales = acuerdo.desafiosIntegracion.reduce(
      (total, desafio) => total + desafio.severidad, 0
    );
   ...
   */
  private calcularInversionInicial(acuerdo: AcuerdoMA): number {
   ...
   */
  private generarEtapasIntegracion(
    negociacion: NegociacionMA,
    acuerdo: AcuerdoMA
  ): { nombre: string; progreso: number; problemas: string[] }[] {
    const etapas = [
      {
  ...
  public actualizarProgreso(
    ...
    if (proceso.estado === EstadoIntegracion.COMPLETADA || 
        proceso.estado === EstadoIntegracion.FALLIDA) {
      return proceso;
    }
    ...
    if (proceso.progreso >= 100) {
      proceso.estado = EstadoIntegracion.COMPLETADA;
   ...
   */
  private actualizarEtapas(
    proceso: ProcesoIntegracion,
    progresoAjustado: number
  ): void {
   ...
   */
  private getProblemasPosiblesPorEtapa(nombreEtapa: string): string[] {
    const problemasPorEtapa: { [key: string]: string[] } = {
