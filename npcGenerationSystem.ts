...
export type CompanyStrategy = 'low_cost' | 'high_quality' | 'niche' | 'balanced' | 'aggressive';
...
export interface NPCCompany {
  ...
  innovationRate: number; // 1-100
  adaptability: number; // 1-100
  
  // Especialización
  preferredCategories: ProductCategory[];
  preferredProducts: string[]; // IDs de productos
  preferredRegions: string[]; // IDs de regiones
  ...
  // Estado
  age: number; // En días de juego
  growthRate: number; // % de crecimiento anual
  status: 'startup' | 'growing' | 'established' | 'declining' | 'struggling';
  
  // IA
  decisionHistory: NPCDecision[];
  lastDecisionTime: number; // timestamp
  
  // Relaciones
  relationWithPlayer: number; // -100 a 100
  competitorRelations: Record<string, number>; // companyId -> relación (-100 a 100)
}

export interface NPCDecision {
  id: string;
  companyId: string;
  type: 'build' | 'produce' | 'buy' | 'sell' | 'expand' | 'contract' | 'price_change' | 'marketing' | 'research';
...
export interface NPCGenerationParams {
  ...
  marketSize: number;
  unsatisfiedDemand: Record<string, number>; // productId -> demanda insatisfecha
  competitorCount: number;
  playerMarketShare: number;
  
  // Parámetros de tiempo
  gameTime: number;
  economicCycle: 'boom' | 'growth' | 'recession' | 'recovery';
  
  // Parámetros de región
  regionId: string;
  regionEconomicStatus: 'poor' | 'developing' | 'wealthy';
  
  // Parámetros de producto
  productCategory?: ProductCategory;
  productTrend?: 'rising' | 'falling' | 'stable';
...
export function generateCompanyName(type: NPCCompanyType, category?: ProductCategory): string {
  const prefixes: Record<NPCCompanyType, string[]> = {
  ...
  const suffixes: Record<NPCCompanyType, string[]> = {
  ...
  const categorySuffixes: Record<ProductCategory, string[]> = {
    [ProductCategory.FOOD]: ['Foods', 'Nutrition', 'Edibles', 'Cuisine', 'Gastronomy', 'Provisions', 'Delicacies', 'Eateries', 'Sustenance', 'Victuals'],
    [ProductCategory.ELECTRONICS]: ['Electronics', 'Devices', 'Circuits', 'Computing', 'Technology', 'Gadgets', 'Hardware', 'Systems', 'Appliances', 'Robotics'],
    [ProductCategory.TEXTILES]: ['Textiles', 'Fabrics', 'Garments', 'Apparel', 'Fashion', 'Clothing', 'Wearables', 'Threads', 'Attire', 'Couture'],
    [ProductCategory.INDUSTRIAL]: ['Industrial', 'Manufacturing', 'Machinery', 'Equipment', 'Tools', 'Construction', 'Engineering', 'Materials', 'Hardware', 'Fabrication'],
    [ProductCategory.CONSUMER_GOODS]: ['Goods', 'Products', 'Essentials', 'Lifestyle', 'Commodities', 'Necessities', 'Merchandise', 'Supplies', 'Accessories', 'Utilities']
  };
...
export function determineCompanyType(params: NPCGenerationParams): NPCCompanyType {
  ...
  Object.keys(adjustedProbabilities).forEach(key => {
    adjustedProbabilities[key as NPCCompanyType] /= total;
  });
  ...
  for (const type of Object.keys(adjustedProbabilities) as NPCCompanyType[]) {
...
export function determineStrategy(type: NPCCompanyType, params: NPCGenerationParams): CompanyStrategy {
  // Probabilidades base para cada estrategia según tipo de compañía
  const baseProbabilities: Record<NPCCompanyType, Record<CompanyStrategy, number>> = {
  ...
  // Ajustar probabilidades según parámetros
  let adjustedProbabilities = { ...baseProbabilities[type] };
  ...
  const total = Object.values(adjustedProbabilities).reduce((sum, prob) => sum + prob, 0);
  Object.keys(adjustedProbabilities).forEach(key => {
    adjustedProbabilities[key as CompanyStrategy] /= total;
  });
  ...
  for (const strategy of Object.keys(adjustedProbabilities) as CompanyStrategy[]) {
...
export function calculateInitialCash(type: NPCCompanyType, params: NPCGenerationParams): number {
  // Valores base por tipo
  const baseCash: Record<NPCCompanyType, number> = {
...
export function determineSpecialization(params: NPCGenerationParams): {
...
export function generateBehaviorTraits(type: NPCCompanyType): {
  ...
  const behaviorRanges: Record<NPCCompanyType, Record<string, [number, number]>> = {
    [NPCCompanyType.STARTUP]: {
...
export function calculateInitialProfitMargin(type: NPCCompanyType, strategy: CompanyStrategy): number {
  // Valores base por estrategia
  const baseMargins: Record<CompanyStrategy, number> = {
    'low_cost': 0.05, // 5%
  ...
  const typeModifiers: Record<NPCCompanyType, number> = {
...
export function calculateInitialReputation(type: NPCCompanyType): number {
  // Valores base por tipo
  const baseReputations: Record<NPCCompanyType, number> = {
...
export function calculateInitialGrowthRate(type: NPCCompanyType, params: NPCGenerationParams): number {
  // Valores base por tipo
  const baseGrowthRates: Record<NPCCompanyType, number> = {
...
export function determineInitialStatus(type: NPCCompanyType): 'startup' | 'growing' | 'established' | 'declining' | 'struggling' {
  // Probabilidades por tipo
  const statusProbabilities: Record<NPCCompanyType, Record<string, number>> = {
    [NPCCompanyType.STARTUP]: {
      'startup': 0.8,
      'growing': 0.2,
      'established': 0.0,
  ...
  for (const status of Object.keys(probabilities) as Array<'startup' | 'growing' | 'established' | 'declining' | 'struggling'>) {
...
export function generateNPCCompany(params: NPCGenerationParams): NPCCompany {
  // Determinar tipo de compañía basado en parámetros
...
export function shouldGenerateNPC(gameState: any): boolean {
...
export function generateParamsFromGameState(gameState: any): NPCGenerationParams {
  // Extraer datos relevantes del estado del juego
  const {
    time,
    difficulty
  ...
  if (playerCompany) {
    ...
    if (marketShares.length > 0) {
      playerMarketShare = marketShares.reduce((sum: number, share: number) => sum + share, 0) / marketShares.length;
    }
  }
  
  // Calcular demanda insatisfecha por producto
  const unsatisfiedDemand: Record<string, number> = {};
  Object.entries(products).forEach(([productId, product]: [string, any]) => {
    if (product.demand > product.supply) {
  ...
  let economicCycle: 'boom' | 'growth' | 'recession' | 'recovery';
  ...
  let regionEconomicStatus: 'poor' | 'developing' | 'wealthy';
  ...
  let productTrend: 'rising' | 'falling' | 'stable' | undefined;
  ...
  const marketSize = Object.values(products).reduce((sum: number, product: any) => 
    sum + (product.demand * product.basePrice.current), 0);
...
export function processDynamicNPCGeneration(gameState: any): NPCCompany | null {
