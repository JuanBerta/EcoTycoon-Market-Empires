...
const regionSystem_1 = require("../engine/entities/regionSystem");
...
const validateEconomicSystem = () => {
    ...
    Object.values(products).forEach((product) => {
        const price = (0, economicSystem_1.calculatePrice)(product.basePrice.current, product.demand, product.supply, globalDemandFactor, globalSupplyFactor, inflationRate);
    ...
    const fluctuations = (0, economicSystem_1.simulateMarketFluctuations)(productsList, globalDemandFactor, globalSupplyFactor, inflationRate);
    fluctuations.forEach(fluctuation => {
        const product = products[fluctuation.id];
...
const validateProductionSystem = () => {
    ...
    recipes.forEach(recipe => {
        ...
        testConfigurations.forEach(config => {
            ...
            const time = (0, productionSystem_1.calculateProductionTime)(recipe, config.efficiency, config.skill, config.tech);
            console.log(`  Tiempo de producción: ${time} horas (base: ${recipe.baseProductionTime})`);
            // Calcular calidad del producto
            const quality = (0, productionSystem_1.calculateProductQuality)(recipe, config.efficiency, config.skill, config.quality);
            ...
            const cost = (0, productionSystem_1.calculateProductionCost)(recipe, config.efficiency, resourceCosts, 1.0);
            console.log(`  Costo de producción: $${cost} (base: $${recipe.baseProductionCost})`);
            // Simular producción completa
            const result = (0, productionSystem_1.simulateProduction)(recipe, config.efficiency, config.skill, config.quality, resourceCosts, 1.0);
...
const validateRegionSystem = () => {
    ...
    // Encontrar ciudades
    const cities = Object.values(locations).filter((loc) => loc.type === 'city');
    ...
    cities.forEach(city => {
        ...
        const demand = (0, regionSystem_1.calculateCityDemand)(city, testProducts);
        ...
        buildingTypes.forEach(type => {
            const cost = (0, regionSystem_1.calculateOperatingCosts)(city, type, 10);
...
const validateNPCSystem = () => {
    ...
    const npcCompanies = Object.values(companies).filter((company) => !company.isPlayer);
    console.log(`Compañías NPC encontradas: ${npcCompanies.length}`);
    // Datos de mercado simulados para pruebas
    const marketData = {
        products: {
            'product-1': { id: 'product-1', price: 20, demand: 70, supply: 50, trend: 'rising' },
            'product-2': { id: 'product-2', price: 100, demand: 50, supply: 60, trend: 'falling' },
            'product-3': { id: 'product-3', price: 40, demand: 60, supply: 60, trend: 'stable' }
        },
        resources: {
    ...
    npcCompanies.forEach(company => {
        ...
        const decisions = (0, npcSystem_1.generateNPCDecisions)(company, marketData, regions, Date.now());
        ...
        decisions.forEach((decision, index) => {
            ...
            const result = (0, npcSystem_1.executeNPCDecision)(decision, company, {});
...
const validateMVP = () => {
    ...
    const allValid = economicResults.pricesValid &&
...
exports.validateMVP = validateMVP;
